...
cwd_wiw_track_R0 <- function (params) {
  ...
  #I know this is a bit redundant, but I thought it would help someone follow my code
  ...
  N <- counts.long %>% 
    ...
    summarize(n = sum(population)) # total pop in final time step
  I <- counts.long %>% 
    filter(year == max(year) &
             str_detect(category, "I")) %>% 
    summarize(n = sum(population)) # total infected in final time step
  
  R0.prev.approx <- as.numeric(1/(1-(I/N))) # prevalence approximation of R0
  
  #these time steps generally align well with
  k <- 36 #36th simulated month
  l <- 48 #48th simulated month
  
  I1 <- counts.long %>% 
    filter(year == unique(year)[k] &
             str_detect(category, "I")) %>% 
    summarize(n = sum(population)) # number of infected at kth time step
  I2 <- counts.long %>% 
    filter(year == unique(year)[l] &
             str_detect(category, "I")) %>% 
    summarize(n = sum(population)) # number of infected at lth time step
  
  r <- as.numeric(log(I2/I1))/(l-k) #this is the exponential rate of growth, divided by the time over which it took place
  ser <- seq(12, 23, length.out = 5) # these are a series of generation times Paul thinks are reasonable
  R0.r.approx <- exp(r*ser) #This comes from https://www.ncbi.nlm.nih.gov/pmc/articles/PMC1766383/
  ...
  output <- list(counts = counts.long, deaths = deaths.long, tracking.inf = tracking.inf.long, R0 = list(R0.point, R0.range)) #prints it out
}
